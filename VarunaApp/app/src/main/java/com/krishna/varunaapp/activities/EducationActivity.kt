...
class EducationActivity : AppCompatActivity() {
    ...
    // -------------------------------------------------------------------
    // OPEN MATERIAL (PDF/IMAGE) — FIXED VERSION
    // -------------------------------------------------------------------
    private fun openMaterial(item: EducationMaterial) {

        val fileName = "temp_${System.currentTimeMillis()}.${item.fileType}"
        val localFile = File(cacheDir, fileName)

        val storageRef = FirebaseStorage.getInstance().getReferenceFromUrl(item.fileUrl)
        ...
        storageRef.getFile(localFile)
            .addOnSuccessListener {
                ...
                try {
                    ...
                    Toast.makeText(this, "No suitable app found to open file", Toast.LENGTH_LONG).show()
                }
            }
...
//
//    private fun openMaterial(item: EducationMaterial) {
//        val bytes = android.util.Base64.decode(item.fileUrl, android.util.Base64.DEFAULT)
//
//        val tempFile = File.createTempFile(
//            "material_",
//            if (item.fileType == "pdf") ".pdf" else ".jpg",
//            cacheDir
//        )
...
//
//        val fileUri = selectedFileUri!!
//        val base64 = convertToBase64(fileUri)
//        val fileType = if (fileUri.toString().contains("pdf")) "pdf" else "image"
//
//        val id = db.collection("education_materials").document().id
//
